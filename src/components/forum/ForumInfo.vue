  ...
  <div>
    <header class="mui-bar mui-bar-nav">
      ...
      <a
        v-show="!isClose"
        @click="closeClick"
        class="calcfont calc-jieshu right"
      ...
      <a
        v-show="!isClose"
        @click="delClick"
        class="calcfont calc-shanchu right"
        :data-AutoID="infoDataList.AutoID"
        id="delete"
      ></a>
    </header>
    <div v-show="!noData" class="pageContent">
      <p
        class="forumTitle"
        :data-AutoID="infoDataList.AutoID"
        :data-TagID="infoDataList.TagID"
      >{{infoDataList.Theme}}</p>
      <div class="forumContentDiv">
        ...
        <div class="containTag">
          <span
            v-for="tagItem in infoDataList.TagNameList"
            :key="tagItem.AutoID"
          >{{tagItem.TagName}}</span>
        </div>
        <div class="file-list">
          <a
            v-for="(item,index) in infoDataList.Attachments"
            :key="index"
            class="file-item f12"
        ...
        <div class="infoStatus">
          <span
            class="info-state"
            :class="{'result73':infoDataList.Result_ID == 73,'result74':infoDataList.Result_ID == 74}"
          >{{infoDataList.Result}}</span>
          <span
            class="info-state"
            :class="{'status71':infoDataList.Status_ID == 71,'status70':infoDataList.Status_ID == 70}"
          >{{infoDataList.Status}}</span>
        </div>
        <div class="feeditemfooter">
          <span class="time">{{infoDataList.PostTime|MeetingTimeFormat}}</span>
          <span class="replies">
            <span class="lanText" data-lanid="1000350_答复"></span>
            <span>{{infoDataList.ReplyCount}}</span>
          </span>
          <span class="hand">
            <span
              class="calcfont"
        ...
        <div class="replyItem" v-for="replyItem in infoDataList.ReplyPosts" :key="replyItem.AutoID">
          ...
          <div class="feeditemfooter">
            ...
            <span class="hand">
              <span
                class="calcfont"
                :class="[parseInt(replyItem.IsCurrentUserDislike)>=1 ? 'calc-caishixin-' : 'calc-cai']"
                :data-statusid="infoDataList.Status_ID"
                :data-autoid="replyItem.AutoID"
                data-even="unfabulous"
                @click="fabulousEvent($event)"
              ></span>
              <span class="ActionCount">{{replyItem.DislikeCount}}</span>
            </span>
            <span class="hand">
              <span
                class="calcfont"
                :class="[parseInt(replyItem.IsCurrentUserLike)>=1 ? 'calc-zan' : 'calc-zan1']"
                :data-statusid="infoDataList.Status_ID"
                :data-autoid="replyItem.AutoID"
                data-even="fabulous"
                @click="fabulousEvent($event)"
              ></span>
              <span class="ActionCount">{{replyItem.LikeCount}}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    closeClick: function() {
      ...
      $("#closeThis")
        ...
        .on("click", function() {
          ...
          jsonDatasTemp["Result"] =
            $("[data-field='Result']").attr("data-fieldVal") || "";
