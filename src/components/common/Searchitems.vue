...
<div class="search-rows f14">
    ...
    <div v-for="item in searchData" :key="item.queryfield" class="item-row">
        ...
        <div v-if="item.fieldControlType =='textareaInput'">
            ...
            <div v-else-if="item.fieldControlType =='dateTimePicker' && item.selectType=='dateSelect'">
                <div class="boxFlex">
                    ...
                    :data-queryComparison="item.queryComparison" />
                        ...
                        <div class="endDate">
                            <input class="dateTimePicker"
                    :data-field="item.nextqueryfield"
                    :data-fieldControlType="item.fieldControlType"
                    :data-TimeType="item.timeType"
                    :TypeValue="item.TypeValue"
                    :data-queryType="item.queryType"
                    ...
                    <div v-else>
                        <div class="item-row-flex">
                            ...
                            <div v-else-if="(item.fieldControlType=='selectList'||item.fieldControlType=='groupSelectList')&&!item.resulteRow" class="selectList row-cell-ft f12" :data-field="item.queryfield" :data-fieldControlType="item.fieldControlType" data-fieldVal="" :data-selectType="item.selectType" :Code="item.Code" :TypeValue="item.TypeValue" :data-queryType="item.queryType" :data-queryFormat="item.queryFormat" :data-queryRelation="item.queryRelation" :data-queryValue="item.queryValue" :data-queryComparison="item.queryComparison" :data-lanid="item.datalanid" :data-fromType="item.fromType"></div>
                  ...
                  />
                            ...
                            <div v-if="(item.fieldControlType=='selectList'||item.fieldControlType=='groupSelectList')&&item.resulteRow" class="resulte-row f12" :data-field="item.queryfield" :data-fieldControlType="item.fieldControlType" data-fieldVal="" :data-selectType="item.selectType" :Code="item.Code" :TypeValue="item.TypeValue" :data-queryType="item.queryType" :data-queryFormat="item.queryFormat" :data-queryRelation="item.queryRelation" :data-queryValue="item.queryValue" :data-queryComparison="item.queryComparison" :data-lanid="item.datalanid" :data-clickObj="item.clickObj"></div>
